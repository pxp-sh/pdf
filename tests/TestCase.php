...
class TestCase extends \PHPUnit\Framework\TestCase
{
     ...
     */
    public static function createFileIO(): \PXP\PDF\Fpdf\IO\FileIO
    {
        return new \PXP\PDF\Fpdf\IO\FileIO(self::getLogger());
    }
     ...
     * @param string|null $outputPath Optional output path for image (default: auto-generated)
    ...
    public static function compareImages(string $imagePath1, string $imagePath2): float
    {
        ...
        if (self::commandExists('compare')) {
            try {
                ...
                if (
                    strpos($e->getMessage(), 'no decode delegate') !== false ||
                    strpos($e->getMessage(), 'ImageMagick compare failed') !== false
                ) {
    ...
    private static function compareImagesWithImageMagick(string $imagePath1, string $imagePath2): float
    {
        ...
        if ($returnCode === 0 && strpos($outputStr, '0 (0)') !== false) {
            return 1.0;
        }

        // Check for specific error messages that indicate ImageMagick can't handle the format
        if (
            strpos($outputStr, 'no decode delegate') !== false ||
            strpos($outputStr, 'no encode delegate') !== false ||
            strpos($outputStr, 'unable to open image') !== false
        ) {
